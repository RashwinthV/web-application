<?php
   ...
   $sql1="SELECT EMAIL FROM registration";
          $res1=$con->query($sql1);
        ...
        <section class="container-fluid">
            <section class="row jumbotron justify-content-center">
                ...
                <?php
                    ...
                    $con = new mysqli('localhost', 'root', '', 'organdb');
                    $qu10 = "SELECT REGISTRATION_ID FROM organ_requests";
                    $rem=$con->query($qu10);

                    $req = mysqli_fetch_assoc($rem);
                    $del=$req;
                    $qu1 = "SELECT * FROM registration";
                    $r=$con->query($qu1);
                    $qu_req = "SELECT * FROM organ_requests";
                                ...
                                if ($Weight >= 50) {
                                    if ($cur_dur >= 5) {
                                        if ($Accident_Type != 'Critical' && $Disease_Type != 'Chronic' && $Operation_Type != 'Major') {
                                            $quantity = $quantity- 1;
                                            if ($quantity == 0) {
                                                $del_qu = "DELETE FROM organ_requests WHERE REGISTRATION_ID = 1";
                                                mysqli_query($con, $del_qu);
                                            } else {
                                                $update_quer = "UPDATE organ_requests SET QUANTITY = $quantity WHERE  REGISTRATION_ID = $req";
                    ...
                    <form action="" method="post" name="myform">
                       ...
                       <div class="mb-4 ">
                        ...
                        <div class="confirm">
                            <label for="confirm" > Comfirm yuor donation</label><br>
                                <input class="confirm" type="radio" name="confirm" id="Radio1" value="confirm">
                                <label class="confirm" for="Radio1">
                                    yes
                                </label>
                            </div>
